...
const dueldadoCommand = async (message, instance, args) => {
    try {
        ...
        if (args && args[0] === 'aceitar') {
            ...
            let msg = `⚔️ *RESULTADO DUELO DO DADO* ⚔️\n\n`;
            ...
            if (duelResult.winner) {
                ...
                msg += `🤝 Ninguém venceu! Nenhuma aposta foi transferida.`;
            }
        ...
        if (!args || args.length < 2) {
            await sendReply(
                ...
                remoteJid,
                '⚔️ *DUELO DO DADO* ⚔️\n\n' +
                'Uso: !dueldado @pessoa [1-6] [reacao/random] [quantidade]\n' +
                'Exemplo: !dueldado @João 4 random 2\n\n' +
                'Para aceitar: !dueldado aceitar [1-6]',
                message.key.id
            );
            return;
        }
        ...
        const result = await startDuel(remoteJid, challengerNumber, challengedNumber, choice, stakeType, stakeAmount, 'dice');
        if (result) {
            await sendReply(instance, remoteJid, result, message.key.id);
            return;
        }
        ...
        await sendReply(
            ...
            remoteJid,
            `⚔️ *DUELO INICIADO!* ⚔️\n\n` +
            `🎯 Número do desafiante: ${choice}\n` +
            `💰 Prêmio: ${amountLabel} ${stakeLabel}\n\n` +
            `@${challengedNumber}, digite:\n!dueldado aceitar [1-6]`,
            message.key.id
        );
    } catch (error) {
        console.error('Error in dueldado command:', error);
        await sendReply(instance, message.key.remoteJid, 'Erro ao processar duelo do dado.', message.key.id);
    }
};

export default dueldadoCommand;
